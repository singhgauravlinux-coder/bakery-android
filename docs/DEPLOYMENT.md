...
## The mobile app's place in this pipeline

`android-app/` is a native Android client — it is **not** a Kubernetes workload, so Argo CD
never deploys it and never will; there's no cluster resource for a compiled APK to become. What
it does share with the 25 backend services is the *promotion policy*, on purpose:

```
.github/workflows/
  _mobile-pipeline.yml   # build+test -> promote-uat -> promote-production (reusable)
  android-app.yml         # thin: "on push to android-app/**, call the pipeline"
```

| Environment | Backend (Argo CD) | Mobile (GitHub Releases) | Gate |
|---|---|---|---|
| dev | auto-sync | APK built + tested on every push, uploaded as a build artifact | none — fastest feedback |
| uat | auto-sync, but only after the "uat" Environment's reviewers approved the tag-bump commit | signed APK attached to a `mobile-uat-<sha>` pre-release | "uat" GitHub Environment reviewers (same people/policy as the backend's) |
| production | **manual** `argocd app sync` on top of the "production" Environment's reviewers | signed `.aab` attached to a `mobile-production-<sha>` release; **manual** Play Console/Fastlane upload still required | "production" GitHub Environment reviewers, then a human ships it |

Each Android build flavor's `API_BASE_URL` (`android-app/app/build.gradle.kts`) is hardcoded to
the exact ingress host that environment's `argocd/<env>-app.yaml` serves
(`dev.bakery.local` / `uat.bakery.local` / `bakery.local`, per `k8s/overlays/<env>/patch-
ingress-host.yaml`) — so there's no separate config file to keep in sync by hand when an
environment's host ever changes; update the one Kustomize patch and the matching flavor block,
in the same PR.

Release signing (`ANDROID_KEYSTORE_BASE64`, `ANDROID_KEYSTORE_PASSWORD`, `ANDROID_KEY_ALIAS`,
`ANDROID_KEY_PASSWORD`) is read from the same `uat`/`production` GitHub Environments the backend
already uses for its reviewer gates — add those as Environment secrets once you have a real
signing key, and `uat`/`production` builds go from debug-signed to properly signed with no other
change.
